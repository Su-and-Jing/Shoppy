          ...
          <div class="buss-insurance cells">
            ...
            <div class="moreInsure" v-show="moreShow">
              ...
              <div class="second" :id="this.typesList[4].code">
                ...
                <van-checkbox
                  v-model="robbingChecked"
                  shape="squre"
                  @change="changeHandle"
                  @click="handleToast2"
                >不计免赔</van-checkbox>
                <van-switch
                  v-model="robbingSwitch"
                  size="24px"
                  @change="changeHandle"
                  @click="handleToast12"
                />
              ...
              <div class="second insures">
                ...
                <van-checkbox
                  v-model="spiritChecked"
                  shape="squre"
                  @change="changeHandle"
                  @click="handleToast5"
                >不计免赔</van-checkbox>
                <div class="insures-right">
                  <input class="spiritInput" v-model="spiritMount" ref="input" @input="handleinput" />
                  <!-- <span>{{spirit}}</span> -->
...
export default {
  ...
  methods: {
    ...
    handleToast9() {
      ...
      } else if (this.carSwitch == false) {
        this.carChecked = true;
      }
    },
    handleToast10() {
      if (this.carSwitch == false && this.damageSwitch == false) {
        this.$toast(
          "此险为车辆损失险（主险）的附加险，请投保车辆损失险（主险）再选择此险！"
        );
      } else if (this.natureSwitch == false) {
    ...
    handleToast5() {
      if (
        this.spiritMount == "" ||
        (this.second == "不投保" && this.spiritPremium != "")
      ) {
        this.$toast(
          "此险为第三者责任险（主险）的附加险，请投保第三者责任险（主险）再选择此险！"
        );
    ...
    handle() {
      ...
      if (data.state == 200 || data.state == 1) {
        ...
        for (var i = 0; i < data.data.kindList.length; i++) {
          ...
          //第三者责任险
          else if (data.data.kindList[i].kindCode == "B") {
          ...
          //司机座位
            ...
            this.fare = data.data.kindList[i].unitAmount;
    ...
    confirmPicker(picker, values) {
      ...
      if (this[this.currentPicker] == "正常缴税") {
        this.payCode == "1";
      } else if (this[this.currentPicker] == "已完税") {
        this.payCode = "2";
      } else if (this[this.currentPicker] == "免税") {
        this.payCode = "3";
      } else if (this[this.currentPicker] == "减税") {
        this.payCode = "4";
